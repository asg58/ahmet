...
const ContextViewerContainer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      // Now try to connect to context namespace
      try {
        // Join context namespace
        ...
        setError(`Error subscribing to context: ${err.message}`);
      }
    });
  ...
  const handleSubscribe = () => {
    if (socket) {
      socket.emit('subscribe', { platform, sessionId });
      console.log(`Subscribed to ${platform} with session ID ${sessionId}`);
      
      // Request the initial context
      socket.emit('getContext', { platform, sessionId });
    }
  };
  ...
  const handleUnsubscribe = () => {
    if (socket) {
      socket.emit('unsubscribe', { platform, sessionId });
      console.log(`Unsubscribed from ${platform} with session ID ${sessionId}`);
      setContext(null);
    }
  };
