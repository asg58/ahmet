...
const ChatContainer: React.FC<{
  initialMessages?: ChatMessageType[]
  sessionId?: string
}> = ({ initialMessages = [], sessionId = uuidv4() }) => {
  ...
  useEffect(() => {
    ...
    if (socketClient.socketInstance) {
      socketClient.socketInstance.emit('ping', null, (response: any) => {
        console.log('Ping response:', response);
        if (response && response.timestamp) {
          console.log('WebSocket connection verified with ping/pong');
        }
      });
    }
    
    // Handle typing indicator
    socketClient.socketInstance?.on('typing', (data: { typing: boolean }) => {
      console.log('Typing indicator:', data);
      setIsLoading(data.typing);
    });
    
    // Handle errors
    socketClient.socketInstance?.on('error', (error: any) => {
    ...
    // Handle session info
    socketClient.socketInstance?.on('session', (data: { sessionId: string }) => {
    ...
    return () => {
      ...
      socketClient.socketInstance?.off('error');
      socketClient.socketInstance?.off('typing');
      socketClient.socketInstance?.off('session');
    };
  }, [sessionId])
...
export default dynamic(() => Promise.resolve(ChatContainer), { ssr: false }) 