...
function displayChatMemory() {
  ...
  try {
    ...
    // Toon AI trigger statistieken als deze beschikbaar zijn
    if (chatMemory.metadata && 
        typeof chatMemory.metadata.aiTriggeredSaves !== 'undefined' &&
        typeof chatMemory.metadata.aiTriggeredReads !== 'undefined') {
...
} else if (process.argv.includes('--trigger-read')) {
  // Voeg AI-getriggerde herlaadactie toe aan de statistieken
  try {
    if (fs.existsSync(MEMORY_FILE)) {
      const data = fs.readFileSync(MEMORY_FILE, 'utf8');
      const chatMemory = JSON.parse(data);
      
      if (chatMemory && chatMemory.metadata) {
        // Verhoog de teller voor AI-getriggerde herladingen
        if (typeof chatMemory.metadata.aiTriggeredReads === 'undefined') {
          chatMemory.metadata.aiTriggeredReads = 1;
        } else {
        ...
        console.log('AI-getriggerde herlaadactie geregistreerd.');
      }
    }
  } catch (error) {
    console.error('Fout bij het registreren van AI-herlaadactie:', error.message);
  }
} 