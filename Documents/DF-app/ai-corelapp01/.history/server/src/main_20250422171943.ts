...
async function bootstrap() {
  try {
    ...
    const app = await NestFactory.create(AppModule, {
      ...
      // Don't stop bootstrap process when there are provider errors
      abortOnError: false
    });
    ...
    // Add global health check endpoint - this runs regardless of other service errors
    ...
    logger.log(`ChromaDB host: ${process.env.CHROMADB_HOST || 'localhost'}`);
    logger.log(`Blender endpoint: ${process.env.BLENDER_BRIDGE_ENDPOINT || 'http://localhost:4201'}`);
    logger.log(`Blender WS endpoint: ${process.env.BLENDER_BRIDGE_WS_ENDPOINT || 'ws://localhost:4202'}`);
  } catch (error) {
