...
export class CodeValidationService {
  ...
  private readonly VALIDATION_MODEL = 'deepseek-coder:7b-q4_K_M';
...
\`\`\``
      ...
      const apiDocs = await this.chromaService.queryApiDocumentation(action, platform, 3);
      if (apiDocs.length > 0) {
        messages.splice(1, 0, {
          role: 'system',
          content: `Relevant API documentation:\n${apiDocs.map(doc => doc.document).join('\n\n')}`
        });
      }
      
      // Use DeepSeek Coder model for validation
      const response = await this.ollamaService.chatCompletionWithModel(
  ...
  async validateCrossPlatform(
    ...
    try {
      ...
      const designConcepts = await this.chromaService.queryApiDocumentation(
...
Provide your analysis in the following JSON format:
{
  ...
  "targetPlatformApproach": "Brief description of how this would be implemented in the target platform"
      ...
      if (designConcepts.length > 0) {
        messages.push({
          role: 'system',
          content: `Cross-platform design concepts:\n${designConcepts.map(doc => doc.document).join('\n\n')}`
        });
      }
