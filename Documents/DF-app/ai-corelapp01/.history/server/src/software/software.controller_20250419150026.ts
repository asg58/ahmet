...
export class SoftwareController {
  ...
  async getBlenderStatus() {
    ...
    try {
      const isConnected = await this.blenderService.checkConnection();
      
      return {
        connected: isConnected,
        version: isConnected ? 'Blender API v1.0' : undefined,
        message: isConnected ? 'Connected to Blender' : 'Not connected to Blender'
      };
    } catch (error) {
