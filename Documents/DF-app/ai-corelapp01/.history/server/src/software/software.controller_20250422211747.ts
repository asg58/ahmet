...
import { SoftwareService, ExecutionResult } from './software.service';
...
export class SoftwareController {
  ...
  async getCorelDrawShapes() {
    ...
    try {
      ...
      // Extract elements from the context (adapt this to match the actual context structure)
      const elements = context.selectedElements || [];
      
      // Transform CorelDRAW shapes into ModelInfo format for the viewer
      const shapes = elements.map((element: any) => ({
  ...
  async exportCorelDrawDocument(@Body() params: {
    ...
    try {
      ...
      const result: ExecutionResult = await this.softwareService.executeAction(
        'coreldraw',
        'exportDocument',
        params,
        undefined // Add the missing conversationContext parameter
      );
      
      return {
        success: result.success,
        fileUrl: result.success && result.returnValue ? result.returnValue.fileUrl : undefined,
      ...
      throw new HttpException(
        `Failed to export document: ${error instanceof Error ? error.message : 'Unknown error'}`,
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
