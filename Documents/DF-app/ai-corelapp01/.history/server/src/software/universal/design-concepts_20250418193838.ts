...
export class DesignConceptMapper {
  ...
  /**
   * Get all available operations for a platform
   * 
   * @param platform The target platform
   * @returns Array of available operation names
   */
  getAvailableOperations(platform: 'coreldraw' | 'blender'): string[] {
    return Object.values(OPERATION_MAPPINGS)
      .filter(mapping => {
        const platformMapping = mapping[platform];
        return platformMapping && 
               (platformMapping.method || platformMapping.code || platformMapping.objectPath);
      })
      .map(mapping => mapping.universalConcept);
  }
  
  /**
   * Get all available object types for a platform
   * 
   * @param platform The target platform
   * @returns Array of available object type names
   */
  getAvailableObjectTypes(platform: 'coreldraw' | 'blender'): string[] {
    return Object.values(OBJECT_TYPE_MAPPINGS)
      .filter(mapping => {
        const platformMapping = mapping[platform];
        return platformMapping && 
               (platformMapping.method || platformMapping.code || platformMapping.objectPath);
