...
export class BlenderService {
  ...
  /**
   * Execute Python code specifically in Blender
   * 
  ...
  async executePythonCode(code: string, options: any = {}): Promise<ExecutionResult> {
    this.logger.debug(`Executing Python code in Blender: ${code.substring(0, 50)}...`);
    
    // In a real implementation, this would send the Python code to Blender for execution
    // For now, we'll just return a mock successful result
    try {
      // For mock implementation, just delegate to the general executeCode method
      const result = await this.executeCode(code, options);
      
      return {
        success: true,
        output: result.output || "Python code executed successfully in Blender",
        data: { executed: true, timestamp: new Date().toISOString() }
      };
    } catch (error) {
      this.logger.error(`Error executing Python code in Blender: ${error.message}`);
      return {
        success: false,
  ...
  /**
   * Execute Python code in Blender (alias for executePythonCode for consistency with SoftwareService)
   */
