...
export class SoftwareController {
  ...
  async getBlenderStatus() {
    try {
      const result = await this.blenderService.getStatus();
      return {
  ...
  async getCorelDrawStatus() {
    try {
      const result = await this.corelDrawService.getStatus();
      return {
        status: result.connected ? 'connected' : 'disconnected',
        details: result
      };
  ...
  async getAvailableSoftware() {
    try {
      ...
      this.logger.error(`Error getting available software: ${error.message}`);
      throw new HttpException(
        `Failed to get available software: ${error.message}`,
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
