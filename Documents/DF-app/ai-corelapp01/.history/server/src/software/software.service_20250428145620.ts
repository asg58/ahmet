    ...
    try {
      ...
      try {
        ...
        if (contextAwareResult.success) {
          ...
          return {
            ...
            returnValue: contextAwareResult.data || contextAwareResult.returnValue
          };
        }
      } catch (error) {
      ...
      try {
        ...
        if (objectModelResult.success) {
          ...
          return {
            ...
            returnValue: objectModelResult.data || objectModelResult.returnValue
          };
        }
      } catch (error) {
      ...
      if (commandResult.success || (commandResult.error && !commandResult.error.includes('Unknown'))) {
        ...
        return {
          ...
          returnValue: commandResult.data || commandResult.returnValue
        };
      }
...
${platform === 'coreldraw' 
  ...
  : 'Genereer Python code voor Blender die bpy (Blender Python API) gebruikt.'}`,
    ...
    const initialCode = codeBlockMatch ? codeBlockMatch[1].trim() : content.trim();
  ...
  async switchPlatform(fromPlatform: string, toPlatform: string): Promise<CommandResult> {
    ...
    try {
      ...
      const totalElements = sourceContext.layers.reduce(
        (count, layer) => count + layer.elements.length, 
        0
      );
      this.logger.debug(`Captured source context with ${totalElements} elements`);
    } catch (error) {
  ...
  async executeNaturalLanguageCommand(
    ...
    this.logger.debug(`Processing natural language command on ${platform}: ${message}`);
    
    try {
      // Get design context if needed
      let context: DesignContext = null;
      if (options.useContext) {
        try {
          context = await this.getDesignContext(platform);
          this.logger.debug(`Context captured for ${platform} with ${context.layers?.length || 0} layers`);
        } catch (error) {
          this.logger.warn(`Failed to capture context for ${platform}: ${error.message}`);
          // Continue without context
        }
      }
      
      // Create an empty chat message array for context
      const chatContext: ChatMessage[] = [];
      
      // If we have an intent, use it directly
      if (options.intent) {
        const result = await this.executeAction(
          platform,
          options.intent,
          {}, // Empty parameters, will be inferred by the action executor
          chatContext
        );
        ...
        return {
          ...
          output: result.success 
            ? `Commando succesvol uitgevoerd: ${result.output || ''}` 
      ...
      // Otherwise, generate and execute command from natural language
      const contextInfo = context ? this.getContextDescription(context) : '';
      
      // Create a structured prompt with the message and context
      const messages: ChatMessage[] = [
        {
          role: 'system',
          content: `Je bent een assistent die ${platform === 'coreldraw' ? 'CorelDRAW' : 'Blender'} commando's genereert. 
          Extraheer de juiste actie en parameters uit de gebruikersopdracht.`
        }
      ];
      ...
      if (contextInfo) {
        messages.push({
          role: 'system',
          content: `Huidige context: ${contextInfo}`
        });
      }
      ...
      // Generate action and parameters
      const response = await this.ollamaService.chatCompletionForTask(
        messages, 
        TaskType.CODE_GENERATION,
        { temperature: 0.2 }
      ...
      const parsed = this.parseGeneratedAction(response.choices[0].message.content);
      
      if (!parsed.action) {
        return {
          success: false,
          output: 'Kon geen actie afleiden uit het commando.',
          error: 'Geen actie gedetecteerd',
          data: null
        };
      }
      ...
      this.logger.error(`Error in natural language command execution: ${error.message}`);
      return {
        success: false,
        output: 'Fout bij uitvoeren commando.',
        error: error.message,
        data: null
      };
    }
  ...
  /**
   * Get a simple text description of the design context
   */
  private getContextDescription(context: DesignContext): string {
    if (!context) return '';
    
    const layers = context.layers?.length || 0;
    const elements = context.layers?.reduce((count, layer) => count + (layer.elements?.length || 0), 0) || 0;
    const selected = context.selectedElements?.length || 0;
    
    return `Document: ${context.documentName}, Layers: ${layers}, Elements: ${elements}, Selected: ${selected}`;
  }
  
  /**
   * Parse generated action from LLM response
   */
  private parseGeneratedAction(content: string): { action: string; parameters: Record<string, any> } {
    try {
      ...
      const actionMatch = content.match(/actie|action: ([\w\d_]+)/i);
