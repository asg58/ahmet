...
export class BlenderContextAnalyzer extends DesignContextAnalyzer {
  ...
  /**
   * Capture context from a Blender instance running in a container
   */
  async captureContextFromContainer(endpoint: string): Promise<DesignContext> {
    this.logger.debug(`Capturing Blender context from container at ${endpoint}`);
    ...
    try {
      ...
      return this.transformBlenderContext(rawContext);
    } catch (error) {
      this.logger.error(`Error capturing Blender context from container: ${error.message}`);
