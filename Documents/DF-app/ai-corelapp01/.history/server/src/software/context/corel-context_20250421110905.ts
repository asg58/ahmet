...
export class CorelContextAnalyzer extends DesignContextAnalyzer {
  ...
  /**
   * Capture context from a CorelDRAW instance running in a container
   */
  async captureContextFromContainer(endpoint: string): Promise<DesignContext> {
    this.logger.debug(`Capturing CorelDRAW context from container at ${endpoint}`);
    
    try {
      // Get context from container API
      const response = await axios.get(`${endpoint}/api/context`);
      
      if (response.status !== 200) {
        throw new Error(`Failed to get context from container: HTTP ${response.status}`);
      }
      
      const rawContext = response.data;
      
      // Transform container context to our standard format
      return this.transformCorelContext(rawContext);
    } catch (error) {
      this.logger.error(`Error capturing CorelDRAW context from container: ${error.message}`);
