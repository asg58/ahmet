...
export class DesignContextAnalyzerService {
  ...
  /**
   * Analyze the design context and return intelligent insights
   * @returns Object containing analyzed context information
   */
  async analyzeCurrentDesignContext() {
    const platform = this.platformSwitching.getCurrentPlatform();
    const contextAnalyzer = platform === 'coreldraw' 
      ? this.corelContextAnalyzer 
      : this.blenderContextAnalyzer;
    
    try {
      const currentContext = await contextAnalyzer.getCurrentContext();
      if (!currentContext) {
        this.logger.warn('No active document or context found');
        return null;
      }

      // Enhance the context with advanced analysis
      const enhancedContext = {
        ...currentContext,
        elementRelationships: await this.analyzeElementRelationships(currentContext),
        stylePatterns: await this.analyzeStylePatterns(currentContext),
        suggestedOperations: await this.suggestRelevantOperations(currentContext),
        documentStructure: await this.analyzeDocumentStructure(currentContext),
        unusualElements: await this.detectAnomaliesInDesign(currentContext),
      };

      this.eventEmitter.emit('design.context.analyzed', enhancedContext);
      return enhancedContext;
    } catch (error) {
      this.logger.error(`Error analyzing design context: ${error.message}`, error.stack);
      return null;
    }
  }

  /**
   * Analyze spatial and hierarchical relationships between elements
   */
  private async analyzeElementRelationships(context) {
    ...
    const allDocumentElements = context.documentElements || [];

    // Analyze relationships between selected elements
    for (let i = 0; i < elements.length; i++) {
      for (let j = i + 1; j < elements.length; j++) {
        const relationship = this.determineElementRelationship(elements[i], elements[j]);
        if (relationship) {
    ...
    // Also analyze relationships between selected elements and key document elements
    for (const selectedElement of elements) {
      for (const docElement of allDocumentElements) {
        if (!elements.includes(docElement)) {
          const relationship = this.determineElementRelationship(selectedElement, docElement);
          if (relationship && this.isSignificantRelationship(relationship)) {
            relationships.push(relationship);
          }
        }
      }
  ...
  /**
   * Determine the relationship between two design elements
   */
  private determineElementRelationship(element1, element2) {
    // Skip if elements don't have necessary properties for comparison
    if (!element1 || !element2 || !element1.position || !element2.position) {
      return null;
    }
    ...
    // Calculate style relationship
    const styleRelationship = this.calculateStyleRelationship(element1, element2);
    
    return {
      elements: [element1.id, element2.id],
      elementTypes: [element1.type, element2.type],
      spatialRelationship,
      styleRelationship,
      hierarchical: this.determineHierarchicalRelationship(element1, element2),
    };
  }

  /**
   * Calculate spatial relationship between elements
   */
  private calculateSpatialRelationship(element1, element2): ElementSpatialRelationship {
    const dx = element2.position.x - element1.position.x;
    const dy = element2.position.y - element1.position.y;
    const distance = Math.sqrt(dx * dx + dy * dy);
    
    // Calculate overlapping
    const e1Right = element1.position.x + (element1.dimensions?.width || 0);
    const e1Bottom = element1.position.y + (element1.dimensions?.height || 0);
    const e2Right = element2.position.x + (element2.dimensions?.width || 0);
    const e2Bottom = element2.position.y + (element2.dimensions?.height || 0);
    
    const isOverlapping = !(
      e1Right < element2.position.x ||
      element1.position.x > e2Right ||
      e1Bottom < element2.position.y ||
      element1.position.y > e2Bottom
    );
    
    // Determine alignment
    const isHorizontallyAligned = Math.abs(element1.position.y - element2.position.y) < 1;
    const isVerticallyAligned = Math.abs(element1.position.x - element2.position.x) < 1;
    
    return {
      distance,
      overlapping: isOverlapping,
      direction: this.getDirection(dx, dy),
      horizontallyAligned: isHorizontallyAligned,
      verticallyAligned: isVerticallyAligned
    };
  }

  /**
   * Calculate style relationship between elements
   */
  private calculateStyleRelationship(element1, element2): ElementStyleRelationship {
    if (!element1.style || !element2.style) {
      return { similarity: 0, matchingProperties: [] };
    }
    ...
    if (element1.style.fillColor && element2.style.fillColor) {
      totalProperties++;
      if (this.colorsAreEqual(element1.style.fillColor, element2.style.fillColor)) {
        matchCount++;
        matchingProperties.push('fillColor');
      }
    }
    
    // Compare stroke properties
    if (element1.style.strokeColor && element2.style.strokeColor) {
      totalProperties++;
      if (this.colorsAreEqual(element1.style.strokeColor, element2.style.strokeColor)) {
        matchCount++;
        matchingProperties.push('strokeColor');
      }
    }
    
    // Compare other style properties
    const otherProperties = ['strokeWidth', 'opacity', 'font', 'fontSize'];
    for (const prop of otherProperties) {
      if (element1.style[prop] !== undefined && element2.style[prop] !== undefined) {
        totalProperties++;
        if (element1.style[prop] === element2.style[prop]) {
          matchCount++;
    ...
    return {
      similarity: totalProperties > 0 ? matchCount / totalProperties : 0,
      matchingProperties
    };
  }

  /**
   * Determine hierarchical relationship between elements
   */
  private determineHierarchicalRelationship(element1, element2) {
    // Check if one element is a parent of the other
    if (element1.children && element1.children.includes(element2.id)) {
      return 'element1_parent';
    }
    if (element2.children && element2.children.includes(element1.id)) {
      return 'element2_parent';
    }
  ...
  /**
   * Get cardinal direction between two elements
   */
  private getDirection(dx, dy) {
   ...
   */
  private colorsAreEqual(color1, color2) {
  ...
  /**
   * Check if a relationship is significant enough to report
   */
  private isSignificantRelationship(relationship) {
    // Consider a relationship significant if:
    // 1. Elements are closely aligned
    if (relationship.spatialRelationship.horizontallyAligned || 
        relationship.spatialRelationship.verticallyAligned) {
      return true;
    }
    
    // 2. Elements have very similar styles
    if (relationship.styleRelationship.similarity > 0.8) {
      return true;
    }
    
    // 3. Elements are hierarchically related
    if (relationship.hierarchical !== 'unrelated') {
      return true;
    }
    
    // 4. Elements are very close to each other
    if (relationship.spatialRelationship.distance < 10) {
      return true;
    }
