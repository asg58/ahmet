...
export class OllamaService implements OnModuleInit {
  ...
  async getBestModelForTask(taskType: TaskType): Promise<string> {
    // Force return "mistral" model for all tasks since it's the only one available
    return "mistral";
  ...
  async streamChatCompletion(request: ChatCompletionRequest, onData: (data: any) => void) {
    try {
      ...
      this.logger.debug(`Using streaming API with ${this.baseUrl}/v1/chat/completions and model ${request.model}`);
