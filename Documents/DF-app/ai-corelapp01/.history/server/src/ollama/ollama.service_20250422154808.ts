...
export class OllamaService {
  ...
  private async initializeAsync() {
    ...
    try {
      this.logger.log(`Attempting to connect to Ollama (attempt ${this.retryCount + 1}/${this.maxRetries})`);
      await this.refreshModels();
      this.isInitialized = true;
      this.connectionError = false;
      this.retryCount = 0; // Reset retry counter on success
      this.logger.log('Successfully connected to Ollama service');
    } catch (error) {
      ...
      if (this.retryCount < this.maxRetries) {
        ...
        this.logger.log(`Will retry Ollama connection in ${delay/1000} seconds`);
  ...
  async chatCompletion(request: ChatCompletionRequest) {
    try {
      ...
      if (!this.isInitialized && !this.connectionError && !this.isInitializing) {
        await this.initializeAsync();
      }
      
      // If service still isn't initialized, throw meaningful error
      if (!this.isInitialized) {
        throw new Error('Ollama service is not initialized, cannot complete request');
      }
