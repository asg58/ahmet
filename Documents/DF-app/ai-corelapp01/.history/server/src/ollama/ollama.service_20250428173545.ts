...
export class OllamaService implements OnModuleInit {
  ...
  private defaultModel = 'mistral-small:3.1';
  ...
  private taskToModelMap: Record<TaskType, string> = {
    [TaskType.CODE_GENERATION]: 'mistral-small:3.1',
    [TaskType.CONTEXT_ANALYSIS]: 'mistral-small:3.1',
    [TaskType.CREATIVE_CONTENT]: 'mistral-small:3.1',
    [TaskType.QUICK_RESPONSE]: 'mistral-small:3.1',
    [TaskType.MATH_REASONING]: 'mistral-small:3.1',
    [TaskType.DEFAULT]: 'mistral-small:3.1',
  };
  ...
  constructor(
    ...
    const port = this.configService.get<string>('OLLAMA_PORT') || '11434';
  ...
  async onModuleInit() {
    this.logger.log('Initializing OllamaService');
    ...
    try {
      ...
      this.logger.log('OllamaService initialized successfully');
    } catch (err) {
      this.connectionError = err.message;
      this.logger.error(`Failed to initialize OllamaService: ${err.message}`);
    } finally {
  ...
  async getStatus(): Promise<{
    ...
    try {
      // Check if Ollama is available using /api/tags
      const pingResult = await firstValueFrom(
        this.httpService.get(`${this.baseUrl}/api/tags`)
      ) as AxiosResponse;
      available = pingResult.status === 200;
    } catch (error) {
      this.logger.warn(`Failed to ping Ollama service: ${error.message}`);
      available = false;
    }
  ...
  async chatCompletion(request: ChatCompletionRequest) {
    try {
      ...
      // Use the OpenAI-compatible API endpoint (/v1/chat/completions)
      this.logger.debug(`Sending chat completion request to ${this.baseUrl}/v1/chat/completions with model ${request.model}`);
  ...
  async streamChatCompletion(request: ChatCompletionRequest, onData: (data: any) => void) {
    try {
      ...
      // Format the messages into a prompt for the older Ollama API
      let prompt = '';
      ...
      // Add a final assistant prompt if the last message was from a user
      if (request.messages.length > 0 && request.messages[request.messages.length - 1].role === 'user') {
        prompt += 'Assistant: ';
      }

      // Use the older API with stream: true
      this.logger.debug(`Using streaming API with ${this.baseUrl}/api/generate and model ${request.model}`);
  ...
  private async checkHealth(): Promise<void> {
    try {
      // Use /api/tags instead of /api/health as the health check endpoint
      const pingResult = await firstValueFrom(
        this.httpService.get(`${this.baseUrl}/api/tags`)
      ) as AxiosResponse;
      ...
      this.logger.error(`Failed to check Ollama service health: ${error.message}`);
    }
  }
} 