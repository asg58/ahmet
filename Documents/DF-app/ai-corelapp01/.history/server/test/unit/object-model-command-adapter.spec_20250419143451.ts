...
describe('ObjectModelCommandAdapter', () => {
  ...
  beforeEach(async () => {
    ...
    const mockCorelDrawObjectModel = {
      ...
      getCurrentContext: jest.fn().mockResolvedValue({
        ...
        selectedObjects: ['Application.ActiveDocument.ActivePage.Shapes.Item(1)'],
        documentProperties: {}
      }),
      getObjectDescriptor: jest.fn(),
      getProperty: jest.fn(),
  ...
  describe('executeCommandViaObjectModel', () => {
    ...
    it('should execute a CorelDRAW rectangle creation command via object model', async () => {
      const result = await adapter.executeCommandViaObjectModel('coreldraw', 'create_rectangle', {
        x: 100,
        y: 200,
        width: 300,
        height: 150
      ...
      expect(corelDrawObjectModel.getCurrentContext).toHaveBeenCalled();
