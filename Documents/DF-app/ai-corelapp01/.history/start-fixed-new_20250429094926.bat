...
:: Verbeterde aanpak - vermijd complexe for loops met pipes
...
powershell -Command "Get-NetTCPConnection -LocalPort 4500 -ErrorAction SilentlyContinue | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force -ErrorAction SilentlyContinue }"
powershell -Command "Get-NetTCPConnection -LocalPort 4501 -ErrorAction SilentlyContinue | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force -ErrorAction SilentlyContinue }"
...
if %ERRORLEVEL% equ 0 (
    ...
    powershell -Command "Invoke-RestMethod http://localhost:4000/api/ollama/models | ForEach-Object { $_.data } | ForEach-Object { Write-Host - $_.name }"
) else (
...
:: Gebruik PowerShell om processen netjes af te sluiten
echo INFO: Bridge processen stoppen...
powershell -Command "Get-NetTCPConnection -LocalPort 4201,4202,4500,4501 -ErrorAction SilentlyContinue | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force -ErrorAction SilentlyContinue }"

:: Stop alle Blender processen die zijn gestart
echo INFO: Blender processen stoppen...
powershell -Command "Get-Process -Name blender -ErrorAction SilentlyContinue | Stop-Process -Force"

:: Stop alle CorelDRAW processen die zijn gestart
echo INFO: CorelDRAW processen stoppen...
powershell -Command "Get-Process -Name CorelDRW -ErrorAction SilentlyContinue | Stop-Process -Force"
