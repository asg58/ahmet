...
async def start_websocket_server():
    ...
    try:
        ...
        result = sock.connect_ex((HOST if HOST != '0.0.0.0' else '127.0.0.1', WEBSOCKET_PORT))
        sock.close()
        
        if result == 0:
            logger.warning(f"Port {WEBSOCKET_PORT} is already in use. WebSocket server will not start.")
            return
...
def start_ws_server():
    ...
    # If debug mode is enabled, don't start the WebSocket server in a separate thread
    # as Flask will restart the application which would cause port conflicts
    if DEBUG:
        logger.warning("Debug mode is enabled. WebSocket server will not start in separate thread.")
        logger.warning("API endpoints will work, but WebSocket functionality will be disabled.")
        return
