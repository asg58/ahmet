...
interface WIN32OLEType {
  client: any;
  connectObject(app: string): any;
}

// Create a mock implementation
const WIN32OLE: WIN32OLEType = {
  client: {
    // Mock methods with appropriate return values
  },
  connectObject: (app: string) => {
    return {
      // Mock CorelDRAW application object
      Visible: true,
      Documents: {
        Add: () => ({}),
      }
...
export class CorelDrawService {
  ...
  initialize(): void {
    if (this.mockMode) {
      this.logger.log('CorelDraw service running in mock mode');
      return;
    }

    try {
      this.corelApp = WIN32OLE.connectObject('CorelDRAW.Application');
      this.logger.log('Successfully connected to CorelDRAW application');
    } catch (error) {
      this.logger.error(`Failed to connect to CorelDRAW: ${(error as Error).message}`);
    }
  }
  ...
  // Fix the error type assertions in these methods
  private handleError(methodName: string, error: unknown): void {
    this.logger.error(`Error in ${methodName}: ${(error as Error).message}`);
  }
} 