...
// In plaats van een rechtstreekse import die kan falen, gebruiken we een dynamic import met fallback
let win32ole: any;

// Mock interface voor win32ole
interface Win32OLE {
  client: {
...
// Als de module niet beschikbaar is, gebruiken we deze mock
const mockWin32ole: Win32OLE = {
  client: {
    Dispatch: (app: string) => {
...
// Probeer de win32ole module te laden, en gebruik de mock als dat niet lukt
try {
  // We gebruiken require ipv import om ervoor te zorgen dat de error in de try/catch block wordt gevangen
  win32ole = require('node-win32ole');
  logger.info('node-win32ole module succesvol geladen');
} catch (error) {
  logger.warn('node-win32ole module niet beschikbaar, fallback naar mock mode');
  win32ole = mockWin32ole;
}
...
export class CorelDrawService {
  ...
  constructor() {
    ...
    // Forceer mock mode als de omgevingsvariabele is ingesteld of als win32ole een mock is
    this.mockMode = process.env.MOCK_MODE === 'true' || win32ole === mockWin32ole;
    
    logger.info(`CorelDRAW Service gestart${this.mockMode ? ' in mock mode' : ''}`);
