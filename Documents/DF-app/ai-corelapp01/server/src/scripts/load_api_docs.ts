...
// Set up ChromaDB connection - use localhost for direct access
...
// Define interface for ChromaDB collection
...
async function main() {
  ...
  if (options.reset) {
    try {
      ...
      // Check if our collection exists before trying to delete it
      for (const collection of collections) {
        // Safely access name property after converting to any type
        const collectionName = (collection as any).name;
        if (collectionName === API_DOCS_COLLECTION) {
  ...
  // Only add embeddingFunction if we're using OpenAI embeddings
  if (embeddingFunction) {
    Object.assign(collectionParams, { embeddingFunction });
  }
  
  const collection = await client.getOrCreateCollection(collectionParams);
