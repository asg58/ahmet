...
export class BlenderService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    this.wsEndpoint = this.configService.get<string>('BLENDER_BRIDGE_WS_ENDPOINT', 'ws://localhost:4203');
    
    // Log the configured endpoints
    this.logger.log(`Blender Bridge REST endpoint configured as: ${this.restEndpoint}`);
    this.logger.log(`Blender Bridge WebSocket endpoint configured as: ${this.wsEndpoint}`);
  }
  
  async onModuleInit() {
    // Connect to WebSocket server
    try {
      console.log('Attempting to connect to Blender WebSocket server at: ' + this.wsEndpoint);
      await this.connectWebSocket();
      console.log('WebSocket connection result completed');
    } catch (error) {
      console.error('Error during WebSocket connection init:', error);
    }
  ...
  private async connectWebSocket(): Promise<void> {
    ...
    try {
      ...
      const endpointsToTry = [
        this.wsEndpoint,
        this.wsEndpoint.replace('localhost', 'host.docker.internal'),
        this.wsEndpoint.replace('host.docker.internal', 'localhost'),
        'ws://localhost:4202',
      ...
      for (const endpoint of endpointsToTry) {
        ...
        try {
          this.logger.debug(`Attempting to connect to WebSocket at: ${endpoint}`);
          ...
          if (connected) {
            // Set up event handlers for successful connection
      ...
      this.logger.error('Failed to connect to any WebSocket endpoint. Falling back to REST API only.');
