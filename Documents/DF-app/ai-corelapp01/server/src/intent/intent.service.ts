...
export class IntentService {
  ...
  /**
   * Determine if we should use the full model for intent detection
   * Always returns false since we're using a simpler model
   */
  private useFullIntentModel(): boolean {
    return false;
  }
    ...
    try {
      ...
      if (options.includeDomainKnowledge !== false) {
        try {
          ...
          this.logger.warn(`Unable to append domain knowledge, continuing without it: ${error.message}`);
        }
      }
      ...
      if (options.useContext !== false) {
        try {
          ...
          this.logger.warn(`Unable to append design context, continuing without it: ${error.message}`);
        }
      }
