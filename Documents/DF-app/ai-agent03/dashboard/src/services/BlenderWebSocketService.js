...
class BlenderWebSocketService {
   ...
   * @param {Object} modelData - Data for the model to create
  ...
  async createModel(modelData) {
    if (!this.connected) {
      ...
      try {
        ...
        console.error('Failed to connect for createModel:', error);
        throw new Error(`Connection failed: ${error.message}`);
      }
    }
    
    return new Promise((resolve, reject) => {
      const messageId = 'create_model_' + Date.now();
...
// Exporteer een instantie van de service in plaats van de klasse
