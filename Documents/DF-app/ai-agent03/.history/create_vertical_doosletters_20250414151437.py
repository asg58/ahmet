#!/usr/bin/env python
# Create vertical doosletters (box letters) in Blender
# Run with: python create_vertical_doosletters.py
...
async def main():
    # Create vertical 3D text in Blender
    vertical_doosletters_script = """
import bpy
import os
import math

# Path voor het opslaan
filepath = os.path.join(os.getcwd(), "vertical_doosletters_scene.blend")
...
# Tekst om te gebruiken - alleen "doosletters"
tekst = "doosletters"
...
print(f"Verticale doosletters scene opgeslagen naar: {filepath}")
"""

    # Send the script to Blender
    await send_bpy_script(SERVER_URI, vertical_doosletters_script)
