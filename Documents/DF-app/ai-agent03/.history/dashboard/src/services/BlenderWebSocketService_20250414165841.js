...
# Zet om naar JSON en print resultaat
...
print(json_result)
    ...
    try {
      ...
      if (result.status === 'ok' && result.details) {
        try {
          // Probeer JSON te extraheren uit de output
          ...
          if (jsonLine) {
            ...
            console.error('Geen JSON data gevonden in Blender output');
            return {
              status: 'error',
              details: 'Geen geometrie data gevonden in Blender output'
            };
          }
        } catch (parseError) {
