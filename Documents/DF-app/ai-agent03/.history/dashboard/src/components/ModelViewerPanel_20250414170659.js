...
import { FaDownload, FaEdit, FaCube, FaInfo, FaExclamationTriangle, FaSync } from 'react-icons/fa';
    ...
    try {
      ...
      if (response.status === 'ok' && response.data && response.data.objects && response.data.objects.length > 0) {
        ...
        if (retryCount < 3) {
          ...
          setLiveStatus({ 
            type: 'warning', 
            message: `No model data found, retrying (${nextRetry}/3)...` 
          });
          
          // Probeer het opnieuw na 3 seconden
          setTimeout(loadLiveModelData, 3000);
        } else {
          setLiveStatus({ 
            type: 'warning', 
            message: 'Could not get model data from Blender after 3 attempts' 
          });
        }
      }
    } catch (err) {
