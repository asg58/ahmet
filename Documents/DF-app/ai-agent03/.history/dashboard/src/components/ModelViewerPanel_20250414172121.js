...
function ModelViewerPanel({ model, onDownload, onEdit }) {
  ...
  const [wsService] = useState(() => new BlenderWebSocketService());
  ...
  const loadLiveModelData = async () => {
    ...
    console.log(`WebSocket status: ${wsService.isConnected ? 'Connected' : 'Disconnected'}`);
    ...
    const attemptLoad = async (attempt = 0) => {
      try {
        ...
        // Use the new getLiveModelData method
        const data = await wsService.getLiveModelData();
        ...
        if (data && data.status === 'ok' && data.data && data.data.objects && data.data.objects.length > 0) {
          ...
          // Process model data using the existing createOrUpdateMesh function from the component
          ...
          Object.keys(modelMeshesRef.current).forEach(name => {
            const stillExists = data.data.objects.some(obj => obj.name === name);
            if (!stillExists) {
