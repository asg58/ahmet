...
function ModelViewerPanel({ model, onDownload, onEdit }) {
  ...
  const createOrUpdateMesh = useCallback((objData) => {
  ...
  const loadLiveModelData = useCallback(async () => {
    setLoading(true);
    setError(null);
    setRetryCount(0);
    ...
    attemptLoad();
  }, [createOrUpdateMesh, blenderService]);
  ...
  }, [liveViewEnabled, pollingInterval, loadLiveModelData]);
