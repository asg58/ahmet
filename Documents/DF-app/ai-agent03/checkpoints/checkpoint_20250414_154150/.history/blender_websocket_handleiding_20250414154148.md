...
```
Dit maakt een nieuwe checkpoint (snapshot) van de huidige projectstatus in de `checkpoints` map.

Om een specifieke benoemde checkpoint te maken:
```
python checkpoint_blender_websocket_v1.py
```
Dit creëert een checkpoint met de vaste naam "blender_websocket_v1_base" in de `checkpoints` map.

Om alle beschikbare checkpoints te bekijken:
```
python create_checkpoint.py list
```
Dit toont een lijst van alle checkpoints met hun datum, aantal bestanden en locatie (checkpoints map of root map voor oudere checkpoints).
...
#### Checkpoint Mapstructuur

Alle checkpoints worden nu in een aparte `checkpoints` map opgeslagen om je project beter georganiseerd te houden:

```
project/
├── checkpoints/                  # Centrale map voor alle checkpoints
│   ├── checkpoint_20250414_153205/  # Een automatisch gegenereerde checkpoint
│   │   ├── blender_agent/
│   │   ├── .history/
│   │   ├── alle bestanden...
│   │   └── checkpoint_info.json  # Metadata over de checkpoint
│   │
│   └── blender_websocket_v1_base/   # Een benoemde checkpoint
│       ├── blender_agent/
│       ├── .history/
│       ├── alle bestanden...
...
- Georganiseerde opslag in de `checkpoints` map
