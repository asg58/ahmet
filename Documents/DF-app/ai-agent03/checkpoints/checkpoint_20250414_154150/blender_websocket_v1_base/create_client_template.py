...
async def send_bpy_script(script):
    """
    ...
    try:
        async with websockets.connect(SERVER_URI) as websocket:
            ...
            message = json.dumps({{"type": "run_script", "script": script}})
            ...
            if response_data.get("status") == "success":
                ...
                print(f"Fout bij uitvoeren script: {{response_data.get('error', 'Onbekende fout')}}")
                
    except ConnectionRefusedError:
        print("Kon geen verbinding maken met de Blender WebSocket server.")
        print("Controleer of de server draait met: python start_blender_server.py")
